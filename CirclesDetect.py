...
def HoughCircles(img):
    # 图片简单处理
    # img = cv2.imread('../test image/4-tiny.jpg')  # 读取图片
    ...
    circles = cv2.HoughCircles(imgray, cv2.HOUGH_GRADIENT, 1, 10000, param1=100, param2=20, minRadius=15,
                               maxRadius=20)  # 霍夫圆变换
    # 第3参数默认为1
    ...
    for i in P:
        # 画出外圆
        cv2.circle(img, (i[0], i[1]), i[2], (0, 255, 0), 3)  # 第二参数（）内是圆心坐标，第三参数是半径，第四参数（）内是颜色，第五参数是线条粗细
        # 画出圆心
        cv2.circle(img, (i[0], i[1]), 2, (0, 0, 255), 3)
    # print("圆的个数是:" + str(len(P)))
    # print("\n")
    ...
    for i in P:
        ...
        x = int(i[0])
        y = int(i[1])
        cv2.putText(img, ('X: '+ str(x) + ' Y: ' +str(y) + 'R: ' + str(r)), (x-50,y-50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2 )
        # print("圆心坐标为：", (x, y))
        # print("圆的半径是：", r)
